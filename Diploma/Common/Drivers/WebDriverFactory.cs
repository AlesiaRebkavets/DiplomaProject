...
namespace Diploma.Common.Drivers
{
    public class WebDriverFactory
    {
        ...
        public static IWebDriver Driver
        {
            get
            {
                if(!DriverCollection.Keys.Contains(TextContextValues.ExecutableClassName))    // if driver is not initialized yet we do it
                {
                    InitializeDriver();
                }

                return DriverCollection.First(pair => pair.Key == TextContextValues.ExecutableClassName).Value; // return driver for needed test class
            }
            private set => DriverCollection.TryAdd(TextContextValues.ExecutableClassName, value); // new driver will be assigned only if DriverCollection doesn't contains value by this key
        }
